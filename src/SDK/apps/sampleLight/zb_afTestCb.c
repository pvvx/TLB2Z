...
static void afTest_testReqPrc(apsdeDataInd_t *pApsdeInd)
{
	...
	dstEp.txOptions = APS_TX_OPT_FRAG_PERMITTED;

	u16 dataLen = pApsdeInd->asduLen;
	u8 *pBuf = (u8 *)ev_buf_allocate(dataLen+2);
...
#else
		af_dataSend(pApsdeInd->indInfo.dst_ep, &dstEp, ZCL_CLUSTER_TELINK_SDK_TEST_RSP, dataLen+2, pBuf, &apsCnt);
#endif
