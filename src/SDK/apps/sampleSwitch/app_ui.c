...
#endif
			/*if(onOff){
				zcl_onOff_onCmd(SAMPLE_SWITCH_ENDPOINT, &dstEpInfo, TRUE);
			}else{
				zcl_onOff_offCmd(SAMPLE_SWITCH_ENDPOINT, &dstEpInfo, TRUE);
			}*/
			onOff ^= 1;
			zcl_onOff_toggleCmd(SAMPLE_SWITCH_ENDPOINT, &dstEpInfo, TRUE);
#else
			brc_toggle();
#endif
		}
