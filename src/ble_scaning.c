...
static void zb_illuminance(u32 lx, u8 n) {
	...
	if(g_zcl_illuminanceAttrs.measuredVal[n] != 0xffff
		&& min_lx != 0xffff) {
		if(ble_illuminance[n] < min_lx) {
			g_zcl_illuminanceAttrs.levelStatus[n] = ILSC_BELOW_TARGET;
		} else if (ble_illuminance[n] == min_lx) {
			g_zcl_illuminanceAttrs.levelStatus[n] = ILSC_ON_TARGET;
		} else {
			g_zcl_illuminanceAttrs.levelStatus[n] = ILSC_ABOVE_TARGET;
		}
	} else {
		g_zcl_illuminanceAttrs.levelStatus[n] = ILSC_NONE;
	}
...
static void zb_moving(u8 n, u8 onoff) {
	if(g_zcl_illuminanceAttrs.minLevelLx[n] != 0
		&& g_zcl_illuminanceAttrs.minLevelLx[n] != 0xffff) {
		if(onoff) {
			// On
