...
static u8 rp_data_enable(u8 endpoint, u16 clusterId, u16 attrId)
{
	u8 ret = 1;
	u8 flg = FLG_UPDATE_NONE;
	if(clusterId == ZCL_CLUSTER_GEN_POWER_CFG) {
		if(attrId == ZCL_ATTRID_BATTERY_VOLTAGE) {
			flg = FLG_UPDATE_VBAT;
		} else if(attrId == ZCL_ATTRID_BATTERY_PERCENTAGE_REMAINING) {
			flg = FLG_UPDATE_BAT;
		} else
			return ret;
	} else if(clusterId == ZCL_CLUSTER_MS_TEMPERATURE_MEASUREMENT) {
		flg = FLG_UPDATE_TEMP;
	} else if(clusterId == ZCL_CLUSTER_MS_RELATIVE_HUMIDITY) {
		flg = FLG_UPDATE_HUMI;
	} else
		return ret;
	ret = update_enable[endpoint-1] & flg;
	return ret;
}
 ...
 */
			if(pEntry->used && (pEntry->maxInterval != 0xFFFF)) {
				...
				if(flg_chk_attr || flg_report) {
					...
					if(rp_data_enable(pEntry->endPoint, pEntry->clusterID, pEntry->attrID) == 0)
						continue;
				}
				if(flg_chk_attr) {
					// report pAttrEntry
					if(zcl_analogDataType(pAttrEntry->type)) {
						...
						len = (len > REPORTABLE_CHANGE_MAX_ANALOG_SIZE) ? (REPORTABLE_CHANGE_MAX_ANALOG_SIZE): (len);
