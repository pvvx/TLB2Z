...
void user_app_init(void)
{
	...
	/* Register endPoint */
	af_endpointRegister(SENSOR_DEVICE_ENDPOINT1, (af_simple_descriptor_t *)&sensorDevice_simpleDesc1, zcl_rx_handler, NULL);
	af_endpointRegister(SENSOR_DEVICE_ENDPOINT2, (af_simple_descriptor_t *)&sensorDevice_simpleDesc2, zcl_rx_handler, NULL);
	af_endpointRegister(SENSOR_DEVICE_ENDPOINT3, (af_simple_descriptor_t *)&sensorDevice_simpleDesc3, zcl_rx_handler, NULL);
//	af_endpointRegister(SENSOR_DEVICE_ENDPOINT4, (af_simple_descriptor_t *)&sensorDevice_simpleDesc4, zcl_rx_handler, NULL);
...
//	zcl_register(SENSOR_DEVICE_ENDPOINT4, SENSOR_DEVICE_CB_CLUSTER_NUM4, (zcl_specClusterInfo_t *)g_sensorDeviceClusterList4);

#if ZCL_OTA_SUPPORT
    ota_init(OTA_TYPE_CLIENT, (af_simple_descriptor_t *)&sensorDevice_simpleDesc1, &sensorDevice_otaInfo, &sensorDevice_otaCb);
#endif
    // read sensors
...
void app_task(void)
{
	...
	if(bdb_isIdle()){
		...
		if(zb_isDeviceJoinedNwk()){
			...
			if(g_sensorAppCtx.reportupsec > READ_SENSOR_TIMER_SEC - 1) {
...
//		bdb_findBindMatchClusterSet(FIND_AND_BIND_CLUSTER_NUM, bdb_findBindClusterList);
		...
		bdb_init((af_simple_descriptor_t *)&sensorDevice_simpleDesc1, &g_bdbCommissionSetting, &g_zbDemoBdbCb, repower);
	} else {
